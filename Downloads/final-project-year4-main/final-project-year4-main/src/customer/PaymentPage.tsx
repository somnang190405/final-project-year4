      ...
      <div className="flex justify-between items-center relative">
        {steps.map((step, idx) => (
          <div key={step.number} className="flex flex-col items-center flex-1">
            {/* Step circle */}
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold mb-2 transition-all border-2 ${
            ...
            {/* Connector line */}
            {idx < steps.length - 1 && (
              <div className={`absolute w-full h-0.5 top-5 left-1/2 transform -translate-x-1/2 transition-all ${
...
const PaymentPage: React.FC<Props> = ({ user, onRequireAuth }) => {
  ...
  const onPay = async (method: 'CARD' | 'PAYPAL' | 'BANK' | 'QR') => {
    ...
    try {
      ...
      navigate('/', { replace: true, state: { toast: { message: 'Payment successful! Your order has been placed.', type: 'success' } } });
    } catch (error: any) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  return (
                    <button
                      ...
                      className={`group p-5 rounded-3xl border-2 transition-all text-left shadow-sm ${
                        selected
                          ? 'border-black bg-black/10 shadow-black/10'
                          : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <span className={`w-11 h-11 rounded-2xl flex items-center justify-center ${selected ? 'bg-black text-white' : 'bg-gray-100 text-gray-600'}`}>
                      ...
                      {selected && <div className="mt-4 text-sm text-gray-900 font-medium">Selected</div>}
                    </button>
                  );
                })}
              </div>
                ...
                <div className="space-y-4 bg-gray-50 p-6 rounded-3xl border border-gray-200 shadow-sm">
                  <div className="rounded-3xl bg-white p-5 border border-gray-200 shadow-sm">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <span className="inline-flex items-center rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-900">Secure</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" 
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      ...
                      <input value={expMonth} onChange={(e) => setExpMonth(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" placeholder="08" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-700 mb-2">Year</label>
                      <input value={expYear} onChange={(e) => setExpYear(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" placeholder="2032" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-700 mb-2">CVV</label>
                      <input value={cvv} onChange={(e) => setCvv(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" placeholder="123" />
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-4 rounded-3xl bg-black text-white font-bold hover:shadow-xl hover:shadow-black/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                  >
                  ...
                  <div className="text-center">
                    {qrDataUrl ? (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="rounded-full bg-gray-100 text-gray-900 px-4 py-2 font-semibold mb-3">Expires in: {expiresText}</div>
              ...
              {paymentMethod === 'BANK' && (
                <div className="bg-gray-50 p-6 rounded-3xl border border-gray-200 shadow-sm space-y-4">
                  ...
                  <div>
                    ...
                    <input value={bankName} onChange={(e) => setBankName(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" placeholder="e.g., ABA" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-900 mb-2">Account Holder Name</label>
                    <input value={accountHolderName} onChange={(e) => setAccountHolderName(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" placeholder="Account holder name" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-900 mb-2">Account Number</label>
                    <input value={accountNumber} onChange={(e) => setAccountNumber(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" placeholder="Enter account number" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-900 mb-2">Confirm Account Number</label>
                    <input value={confirmAccountNumber} onChange={(e) => setConfirmAccountNumber(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" placeholder="Confirm account number" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-900 mb-2">Transfer Reference (optional)</label>
                    <input value={transferReference} onChange={(e) => setTransferReference(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-black focus:ring-2 focus:ring-black/10 outline-none transition" placeholder="Reference or note" />
                  </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl p-4 border border-gray-200 sticky top-8">
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-3xl font-bold text-gray-900">{fmtMoney(total)}</span>
              </div>
              ...
              <button
                ...
                className="w-full py-4 rounded-3xl bg-black text-white font-bold text-lg hover:shadow-xl hover:shadow-black/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all transform hover:scale-105 hover:disabled:scale-100 mb-4 flex items-center justify-center gap-2"
              >
                {busy ? (
                  <>
