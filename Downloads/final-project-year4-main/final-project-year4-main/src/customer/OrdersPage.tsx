  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      {loading ? (
                ...
                {pendingReturnOrders.map((order) => {
                  ...
                  return (
                    <div key={order.id} className="rounded-3xl border border-yellow-200 bg-yellow-50 p-5 shadow-sm">
                      ...
                      {(order.items || []).filter((item: any) => item?.image).slice(0, 3).length > 0 && (
                        <div className="mb-4 flex items-center gap-3 overflow-x-auto">
                          {(order.items || []).filter((item: any) => item?.image).slice(0, 3).map((item: any, idx: number) => (
                            <div key={idx} className="h-16 w-16 shrink-0 overflow-hidden rounded-3xl border border-gray-200 bg-gray-50">
                              <img src={item.image} alt={item.name || 'Product'} className="h-full w-full object-cover" loading="lazy" decoding="async" onError={(e) => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden'; }} />
                            </div>
                          ))}
                        </div>
                      )}
        ...
        <div className="max-w-xl mx-auto rounded-3xl border border-gray-200 bg-white p-10 text-center shadow-sm">
          <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-indigo-50 text-indigo-600">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 17v-6a2 2 0 012-2h2a2 2 0 012 2v6m2 0H7m10 0a5 5 0 10-10 0h10z" />
            </svg>
          </div>
          <h2 className="text-2xl font-semibold text-gray-900 mb-3">No orders yet?</h2>
          <p className="text-gray-600 mb-6">Your next favorite outfit is waiting. Start shopping now and your order history will appear here.</p>
          <Link to="/shop" className="inline-flex items-center justify-center rounded-3xl bg-black px-6 py-3 text-sm font-semibold text-white hover:bg-gray-900 transition">
            Start Shopping
          </Link>
        </div>
      ) : (
        <div className="flex flex-col gap-8">
          {rows.map((order) => {
            ...
            const orderNo = `#${String(order.id).slice(0, 10)}`;
            const stepIndex = status === 'Delivered' ? 3 : status === 'Shipped' ? 2 : status === 'Pending' ? 1 : 0;
            const progressSteps = [
              { label: 'Ordered', active: stepIndex >= 1 },
              { label: 'Shipped', active: stepIndex >= 2 },
              { label: 'Delivered', active: stepIndex >= 3 },
            ];
            const previewImages = (order.items || []).filter((item: any) => item?.image).slice(0, 3);
                    ...
                    {progressSteps.map((step, index) => (
                      <React.Fragment key={step.label}>
                        <div className="flex items-center gap-2">
                          <div className={`h-3.5 w-3.5 rounded-full ${step.active ? 'bg-black' : 'bg-gray-300'}`}></div>
                          <span className={`text-xs ${step.active ? 'text-gray-900' : 'text-gray-400'}`}>{step.label}</span>
                        </div>
                        {index < progressSteps.length - 1 && <div className={`h-px flex-1 ${progressSteps[index].active ? 'bg-black' : 'bg-gray-200'}`}></div>}
                      </React.Fragment>
                    ))}
                  </div>
                  {previewImages.length > 0 && (
                    <div className="flex items-center gap-3 overflow-x-auto pb-2">
                      {previewImages.map((item: any, idx: number) => (
                        <div key={idx} className="h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-gray-200 bg-gray-50">
                          <img src={item.image} alt={item.name || 'Product'} className="h-full w-full object-cover" loading="lazy" decoding="async" onError={(e) => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden'; }} />
                        </div>
                      ))}
                    </div>
